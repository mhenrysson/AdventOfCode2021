class EightAdvent:
    ...
    @staticmethod
    def first():
        inputs = [a.split("|") for a in EightAdvent.get_input().splitlines()]
        displays = [a[1].strip().split(" ") for a in inputs]
        count = sum([1 for display in displays for digit in display if len(digit) < 5 or len(digit) == 7])
        print("First: ", count)

    @staticmethod
    def second():
        inputs = [a.split("|") for a in EightAdvent.get_input().splitlines()]
        signals = [a[0].strip().split(" ") for a in inputs]
        displays = [a[1].strip().split(" ") for a in inputs]
        results = []
        for i in range(len(signals)):
            letters = EightAdvent.extract_letters(signals[i])
            digits = ''.join([l for d in displays[i] for l in letters
                              if len(d) == len(letters[l])
                              and all([dd in letters[l] for dd in d])])
            results.append(int(digits))
        print("Second: ", sum(results))

    @staticmethod
    def extract_letters(signal):
        digits = {"1": [d for s in signal for d in s if len(s) == 2],
                  "7": [d for s in signal for d in s if len(s) == 3],
                  "4": [d for s in signal for d in s if len(s) == 4],
                  "8": [d for s in signal for d in s if len(s) == 7]}
        digits["3"] = [d for s in signal for d in s
                       if len(s) == 5
                       and all([i in s for i in digits["1"]])]
        digits["9"] = [d for s in signal for d in s
                       if len(s) == 6
                       and all([i in s for i in digits["3"]])]
        digits["0"] = [d for s in signal for d in s
                       if len(s) == 6
                       and all([i in s for i in digits["7"]])
                       and not all([i in s for i in digits["9"]])]
        digits["6"] = [d for s in signal for d in s
                       if len(s) == 6
                       and not all([i in s for i in digits["0"]])
                       and not all([i in s for i in digits["9"]])]
        digits["5"] = [d for s in signal for d in s
                       if len(s) == 5
                       and all([i in digits["6"] for i in s])]
        digits["2"] = [d for s in signal for d in s
                       if len(s) == 5
                       and not all([i in s for i in digits["5"]])
                       and not all([i in s for i in digits["3"]])]
        return digits
...
gcafb gcf dcaebfg ecagb gf abcdeg gaef cafbge fdbac fegbdc | fgae cfgab fg bagce"""
    ...
    @staticmethod
    def get_input():
        return """cdbga acbde eacdfbg adbgf gdebcf bcg decabf cg ebdgac egca | geac ceag faedcb cg
