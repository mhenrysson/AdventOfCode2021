import copy


def main():
    print("First test: ", first(get_test_input()))
    print("First real: ", first(get_input()))
    print("Second test: ", second(get_test_input()))
    print("Second real: ", second(get_input()))


def first(inputs: str):
    data = inputs.splitlines()
    trees = []
    for a in data:
        _, tree = generate_branch(a[1:])
        trees.append(tree)
    trees = [do_reduce(tree) for tree in trees]
    fin = trees[0]
    for tree in trees[1:]:
        fin = do_reduce([fin, tree])
    return get_magnitude(fin)


def second(inputs: str):
    data = inputs.splitlines()
    trees = []
    m = 0
    for a in data:
        _, tree = generate_branch(a[1:])
        trees.append(tree)
    trees = [do_reduce(tree) for tree in trees]
    for i in range(len(trees)):
        for j in range(len(trees)):
            if i != j:
                fin = do_reduce([copy.deepcopy(trees[i]), copy.deepcopy(trees[j])])
                m = max(m, get_magnitude(fin))
    return m


def generate_branch(a: str):
    b = []
    for _ in range(2):
        if a[0] == '[':
            a, c = generate_branch(a[1:])
            b.append(c)
        else:
            b.append(int(a[0]))
            a = a[1:]
        a = a[1:]
    return a, b


def do_reduce(a):
    c = True
    while c:
        c, _, _ = do_explode(a, 1)
        if not c:
            c = do_split(a)
    return a


def do_explode(tree, depth):
    if depth == 5:
        return True, tree[0], tree[1]
    if type(tree[0]) is list:
        c, x, y = do_explode(tree[0], depth + 1)
        if c:
            if x >= 0 and y >= 0:
                tree[0] = 0
            if y >= 0:
                if type(tree[1]) is int:
                    tree[1] += y
                else:
                    add_left(tree[1], y)
            return True, x, -1
    if type(tree[1]) is list:
        c, x, y = do_explode(tree[1], depth + 1)
        if c:
            if x >= 0 and y >= 0:
                tree[1] = 0
            if x >= 0:
                if type(tree[0]) is int:
                    tree[0] += x
                else:
                    add_right(tree[0], x)
            return True, -1, y
    return False, -1, -1


def do_split(tree):
    for i in [0, 1]:
        if type(tree[i]) is list:
            if do_split(tree[i]):
                return True
        elif tree[i] > 9:
            tree[i] = [int(tree[i] / 2), int(tree[i] / 2) + tree[i] % 2]
            return True
    return False


def add_left(tree, y):
    if type(tree[0]) is int:
        tree[0] += y
    else:
        add_left(tree[0], y)


def add_right(tree, x):
    if type(tree[1]) is int:
        tree[1] += x
    else:
        add_right(tree[1], x)


def get_magnitude(tree):
    if type(tree) is int:
        return tree
    else:
        return 3 * get_magnitude(tree[0]) + 2 * get_magnitude(tree[1])


def get_test_input():
    return """[[[0,[5,8]],[[1,7],[9,6]]],[[4,[1,2]],[[1,4],2]]]
[[[5,[2,8]],4],[5,[[9,9],0]]]
[6,[[[6,2],[5,6]],[[7,6],[4,7]]]]
[[[6,[0,7]],[0,9]],[4,[9,[9,0]]]]
[[[7,[6,4]],[3,[1,3]]],[[[5,5],1],9]]
[[6,[[7,3],[3,2]]],[[[3,8],[5,7]],4]]
[[[[5,4],[7,7]],8],[[8,3],8]]
[[9,3],[[9,9],[6,[4,9]]]]
[[2,[[7,7],7]],[[5,8],[[9,3],[0,2]]]]
[[[[5,2],5],[8,[3,7]]],[[5,[7,5]],[4,4]]]"""


def get_input():
    return """[[8,[6,[0,1]]],[8,[2,[1,9]]]]
[[[5,[1,7]],9],[4,[[9,4],4]]]
[[[1,[6,3]],8],[[3,[5,3]],[1,6]]]
[0,[[1,[7,4]],[6,[7,3]]]]
[[[7,6],2],[[[9,5],9],[[5,5],[6,3]]]]
[[3,[[2,3],0]],0]
[4,[2,[4,[2,6]]]]
[[2,[8,[4,5]]],[[[7,7],7],1]]
[[[[6,7],6],[9,8]],[[0,[7,3]],[[9,1],[2,0]]]]
[0,[[[8,4],[4,9]],5]]
[[0,7],[8,5]]
[6,[[9,[0,7]],[[0,0],[8,1]]]]
[[[[8,3],[1,9]],[[9,3],[6,5]]],4]
[[[6,6],[[1,2],[1,7]]],[[8,8],[3,2]]]
[[[6,4],[[0,3],1]],[[6,2],[4,[0,3]]]]
[[[2,9],[[2,1],1]],[[6,[1,4]],[6,[0,3]]]]
[9,[[7,4],[1,9]]]
[[[[1,2],[7,8]],[[9,6],[1,3]]],[[0,6],[[3,6],6]]]
[[[1,[8,6]],[2,[3,4]]],[[0,4],[5,[5,7]]]]
[[[5,9],[[1,0],[4,3]]],[[7,[6,7]],[1,[1,5]]]]
[[[[6,8],[5,9]],[5,[4,5]]],4]
[[[[3,4],4],7],[[5,[3,7]],7]]
[[[[3,3],[7,9]],[1,[4,8]]],0]
[[[3,[9,4]],[1,[3,7]]],[[[1,8],1],[6,1]]]
[3,[[[7,5],[4,8]],[7,8]]]
[[7,[2,[2,4]]],[0,[8,[0,3]]]]
[[[[8,5],3],[3,[8,3]]],[1,[0,[7,4]]]]
[[[[7,1],3],[3,4]],[[3,7],[[1,8],[4,8]]]]
[[[3,[9,9]],4],[[4,2],[[4,2],4]]]
[[[5,[9,1]],[[3,5],[1,9]]],7]
[[[[0,8],5],[9,[5,1]]],[[7,0],1]]
[[[0,2],[[1,9],7]],[[0,3],[[0,3],[4,8]]]]
[[[[1,8],0],[[8,6],[7,6]]],[[[1,8],4],[[0,4],[8,3]]]]
[[[1,[2,7]],[[5,4],[5,0]]],[5,[8,[8,4]]]]
[[[[4,4],[7,3]],[4,[2,3]]],[[[6,5],[1,5]],[5,[8,6]]]]
[[[[7,8],4],[9,[4,2]]],[[[1,4],2],[0,7]]]
[[8,4],[1,[2,5]]]
[[[[2,5],4],[7,[0,2]]],[5,3]]
[[3,[[7,4],3]],3]
[[[3,5],[3,[1,4]]],[[[0,8],1],8]]
[[[[1,9],5],[2,[4,8]]],[[[9,2],[0,1]],1]]
[[[6,[1,5]],[[2,2],6]],[[1,[2,6]],5]]
[[[3,2],[9,3]],[[2,1],[4,8]]]
[[[[9,2],7],[[5,9],[1,2]]],[[[3,0],[2,8]],0]]
[[[6,5],[[9,4],3]],[[[6,2],1],[0,7]]]
[[[8,6],1],[9,[1,[0,1]]]]
[[[[5,1],4],[8,[6,8]]],[4,[[1,8],9]]]
[[[[1,1],[8,9]],[2,[0,6]]],3]
[[[1,[8,3]],[[4,3],1]],[[[4,1],[8,6]],8]]
[[8,[[6,2],8]],[[[4,0],8],6]]
[[[[2,2],7],[[9,0],[3,3]]],[[[4,4],0],2]]
[8,[[3,[9,1]],[0,[9,1]]]]
[[[0,[4,2]],[[2,2],[8,7]]],[[6,[4,2]],[1,6]]]
[[3,2],[4,[[6,2],2]]]
[[6,[3,[2,9]]],[[9,[1,5]],[4,4]]]
[[[[7,5],5],8],[1,[0,[2,7]]]]
[[2,[[2,9],[1,6]]],[[[0,1],[0,2]],[4,[3,4]]]]
[[[[8,9],[7,4]],[8,[6,5]]],1]
[[8,9],[[2,[6,9]],[2,8]]]
[[5,1],8]
[[[8,[4,2]],[5,[1,8]]],[[0,[0,6]],[[6,7],9]]]
[[[8,[8,0]],[[8,0],8]],[[[9,9],9],[9,[5,4]]]]
[[[[3,3],5],[5,[9,0]]],[[2,6],[[3,8],[7,1]]]]
[3,[[[1,5],8],5]]
[[[9,8],[4,3]],5]
[[[5,7],[[2,1],6]],[[4,2],[1,[0,2]]]]
[[[[9,3],[9,8]],[[1,0],6]],[[[6,5],2],[[0,3],6]]]
[8,[[[9,8],[2,8]],[1,0]]]
[[8,[5,9]],[[[4,3],6],[[5,1],4]]]
[[0,8],[1,[4,[6,3]]]]
[3,[3,[6,[5,6]]]]
[0,[0,[[8,0],8]]]
[[0,4],[[7,4],[[0,7],1]]]
[7,[[[6,3],[4,0]],1]]
[9,[5,[[5,3],[2,8]]]]
[[7,[[8,3],[1,7]]],[[[2,7],1],[[9,4],[7,1]]]]
[[[0,[7,3]],3],2]
[[1,[[9,0],2]],3]
[[1,[7,[0,1]]],[[1,8],5]]
[3,[5,[4,1]]]
[3,[[[9,8],4],[4,[9,7]]]]
[[2,9],[0,9]]
[[[[7,1],[9,3]],[1,[1,8]]],9]
[[[9,8],[[7,8],3]],[[1,[6,3]],[2,[7,3]]]]
[[[7,3],[1,[5,5]]],[[4,8],[8,[2,5]]]]
[[2,[[6,5],[4,6]]],[[0,3],7]]
[[[4,[9,7]],[[6,1],6]],[[[8,1],6],[[2,5],9]]]
[[[6,0],0],[9,9]]
[[[[1,0],0],[[5,7],9]],[[[7,2],0],[9,6]]]
[[[[5,0],[2,0]],[0,[7,5]]],[[[7,7],[2,4]],8]]
[0,[[9,[3,4]],[[3,4],6]]]
[[[0,8],[[1,5],[3,4]]],[[5,[6,4]],[[2,5],[2,5]]]]
[[8,0],[[2,[7,9]],9]]
[[[3,[7,0]],[3,[8,4]]],2]
[[8,1],[[[8,9],[1,0]],3]]
[[[8,3],[[4,8],4]],[[8,[8,8]],[0,2]]]
[[0,[9,4]],[[6,8],[[7,1],9]]]
[[[[5,3],[2,8]],[8,7]],[9,[[5,9],[5,2]]]]
[2,[4,[[4,3],8]]]
[[[[7,2],[6,4]],7],8]"""


if __name__ == '__main__':
    main()
